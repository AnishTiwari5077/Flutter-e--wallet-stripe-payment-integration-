...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<void> _loadData() async {
    ...
    // Refresh UserEntity data from server
    await auth.refreshUser();

    // Fetch transactions if UserEntity exists
    if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    // No UserEntity State - But only show if NOT logging out
  ...
  Widget _buildTransactionItem(TransactionEntity tx, int userId) {
    final type = tx.transactionType.toLowerCase();
    final amount = tx.amount;
    final date = tx.formattedDateTime;
    ...
    switch (type) {
      ...
      case 'send':
        if (isSent) {
          ...
          color = Colors.red;
          final receiverName = tx.receiverName;
          ...
          color = Colors.green;
          final senderName = tx.senderName;
