...
class _TransactionHistoryScreenState extends State<TransactionHistoryScreen> {
  ...
  Future<void> _showTransactionModel(TransactionEntity tx) async {
    try {
      ...
      // Receipt is just the transaction entity itself
      final receipt = tx;
  ...
  Widget _buildTransactionCard(TransactionEntity tx, int userId) {
    final type = tx.transactionType.toLowerCase();
    final amount = tx.amount;
    final date = tx.dateTime.toIso8601String();
    final isSent = tx.senderId == userId;
    ...
    switch (type) {
      ...
      case 'send':
        if (isSent) {
          ...
          color = Colors.red;
          title = 'Sent to ${tx.receiverName ?? 'Unknown'}';
          subtitle = tx.receiverPhone ?? '';
        } else {
          icon = Icons.arrow_downward;
          color = Colors.green;
          title = 'Received from ${tx.senderName ?? 'Unknown'}';
          subtitle = tx.senderPhone ?? '';
        }
        break;
      case 'bank_transfer':
        ...
        children: [
          ...
          Column(
            ...
            children: [
              ...
              // ✅ RECEIPT BUTTON
              InkWell(
                onTap: () => _showTransactionModel(tx),
